<div x-data="quillEditor()" x-init="initEditor()" @quill-change.window="updateContent($event)">
    ...
    <form wire:submit.prevent="save">
            ...
            @foreach($form->images as $index => $image)
                <div class="mb-2 flex justify-center">
                    <div class="flex flex-col justify-between h-full">
                        <div>
                            <img src="{{ asset($image['url']) }}" alt="Uploaded Image" class="max-w-32 mx-auto h-auto rounded">
                            <button type="button" wire:click="deleteImage({{ $index }}, {{ $image['id'] }})" wire:confirm="Sei sicuro di voler rimuovere questa immagine?" class="my-2 bg-red-700 text-white rounded-md font-semibold text-xs w-full py-2 uppercase tracking-widest">Rimuovi</button>
                        </div>
                        <div class="mt-auto">
        ...
        <!-- Video Links Section -->
        <div class="mt-4 mb-4 flex md:w-1/2">
            <input
                class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 text-xs text-gray-900"
                type="text"
            ...
            @foreach($form->videos as $index => $video)
                <div class="mb-2 flex justify-center">
                    <div>
                        ...
                        <button type="button" wire:click="removeVideo({{ $index }}, {{ $video['id'] }})" wire:confirm="Sei sicuro di voler rimuovere questo video?" class="mb-1 bg-red-700 text-white rounded-md font-semibold text-xs w-full py-1 uppercase tracking-widest">Rimuovi</button>
                    </div>
                </div>
            @endforeach
        </div>
